...
const pt45Start = testStarts.find(t => t.num === 45).index;
const pt45End = testStarts.find(t => t.num === 46).index;
const pt45Content = txtContent.substring(pt45Start, pt45End);

const pStarts = [];
const pRegex = /^(Questions?\s+\d+[-–]\d+|Passage\s+\d+)/gmi;
let pMatch;
while ((pMatch = pRegex.exec(pt45Content)) !== null) {
...
for (let j = 0; j < pStarts.length; j++) {
  ...
  const end = j + 1 < pStarts.length ? pStarts[j + 1].index : pt45Content.length;
  groups.push({ header: pStarts[j].header, content: pt45Content.substring(start, end) });
}

for (let gIdx = 0; gIdx < groups.length; gIdx++) {
  const group = groups[gIdx];
  console.log(`\n=== PT45 Group ${gIdx + 1} (${group.header}) ===`);
